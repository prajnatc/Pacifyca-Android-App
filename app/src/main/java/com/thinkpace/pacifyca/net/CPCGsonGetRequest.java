...
public class CPCGsonGetRequest extends Request<IPCDataModel> {

    private final Listener<IPCDataModel> mListener;
    private IPCDataModel mDataModel;
    private final Gson mGson;
    private Map<String, String> mHeaders;
    private String TAG = CPCGsonGetRequest.class.getName();
    ...
    public CPCGsonGetRequest(String url, Listener<IPCDataModel> listener, ErrorListener errorListener, IPCDataModel model) {
    ...
    public CPCGsonGetRequest(String url, Listener<IPCDataModel> listener, ErrorListener errorListener, IPCDataModel model, Map<String, String> headers) {
        super(Method.GET, url, errorListener);
        mListener = listener;
        mDataModel = model;
        mGson = new Gson();
    ...
    protected Response<IPCDataModel> parseNetworkResponse(NetworkResponse response) {
        ...
        CPCAppCommonUtility.log(TAG, "URL: " + mUrl);
        CPCAppCommonUtility.log(TAG, "json Response String: " + jsonString);
        int statusCode = response.statusCode;
        try {
            if (statusCode == ERROR_CODE_401 || statusCode == ERROR_CODE_410) {
                throw new IllegalErrorCode(jsonString);
            }
            if (statusCode == ERROR_CODE_499 || statusCode == ERROR_CODE_502
            ...
            if ((statusCode == 404 || statusCode == 503)) {
                ...
                /*statusCode = 503;
                throw new IllegalErrorCode(jsonString);*/
            } else return Response.success((IPCDataModel)
                    mGson.fromJson(jsonString, mDataModel.getClass()), getCacheEntry());
